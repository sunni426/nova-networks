...
class CellSegmentator(object):
    ...
    def __init__(
        ...
        if isinstance(nuclei_model, str):
            print("Model weights exist: ", os.path.exists(nuclei_model))
        ...
        self.nuclei_model = nuclei_model.to(self.device) #ERROR, model exist but cannot take imgs
    ...
    def _image_conversion(self, images):
        ...
        # print(f'microtubule_imgs is {microtubule_imgs}')
        # print(f'er_imgs is {er_imgs}')
        # print(f'nuclei_imgs is {nuclei_imgs}')
        microtubule_imgs = [microtubule_imgs]
        er_imgs = [er_imgs]
        nuclei_imgs = [nuclei_imgs]
        if self.multi_channel_model:
            if not isinstance(er_imgs, list):
                print("ERROR:", er_imgs) # added, Sunni
    ...
    def pred_nuclei(self, images):
        ...
        def _preprocess(image):
            ...
            #print('IMAGE PREPROCESS', image)
            # rescaling image to 1/4 size (scale factor==0.25)
            image = transform.rescale(image, self.scale_factor, channel_axis=-1) #removed multichannel=True
