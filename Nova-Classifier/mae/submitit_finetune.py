...
def get_shared_folder() -> Path:
    ...
    # print(os.getcwd())
    path = f'{os.getcwd()}/$..' # ""/$../results/e89/checkpoints/"
    if Path(path).is_dir():
        p = Path(path)
        p.mkdir(exist_ok=True)
        return p
...
#     def __call__(self):
#         self.args.output_dir = self.args.output_dir.replace("%j", str(uuid.uuid4().hex))
#         self.args.log_dir = self.args.output_dir
#         self.args.gpu = 9  # Set GPU directly
#         self.args.rank = 0
#         self.args.world_size = 1
#         print(f"Process group: 1 task, rank: 0")
...
#         self.args.log_dir = self.args.output_dir
#         self.args.gpu = 9 # job_env.local_rank # CHANGE HERE FOR GPU!
...
#     os.environ["CUDA_VISIBLE_DEVICES"] = "9"  # Assuming GPU 9 is assigned to the job
