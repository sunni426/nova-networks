...
def basic_train(
    ...
    try:
        ...
        for epoch in range(cfg.train.num_epochs):
            ...
            for i, (ipt, mask, lbl, cnt) in enumerate(tq):
                ...
                if (
                    ...
                    and r < cfg.train.cutmix_prob
                ):
                    input, target_a, target_b, lam_a, lam_b = cutmix(
                        ipt, lbl, img_size, cfg.train.beta, model
                    )
                    cell, exp = model(ipt, cfg.experiment.count)
                    # print(cell.shape, lam_a.shape)
                    cell_loss = torch.nn.BCEWithLogitsLoss(
                        pos_weight=pos_weight, reduction="none"
                    )
                    # print(cell.shape, lam_a.shape, cell_loss(cell, target_a).shape)
                    loss_cell = (
                        cell_loss(cell, target_a).mean(1)
                        * torch.tensor(lam_a).cuda().float()
                        + cell_loss(cell, target_b).mean(1)
                        * torch.tensor(lam_b).cuda().float()
                    )
                    ...
                    loss_exp = (
                        loss_func(exp, target_a_exp).mean(1)
                        * torch.tensor(lam_a_exp).cuda().float()
                        + loss_func(exp, target_b_exp).mean(1)
                        * torch.tensor(lam_b_exp).cuda().float()
                    )
                    ...
                    if cfg.basic.amp == "Native":
                        with torch.cuda.amp.autocast():
                            ...
                            loss_cell = torch.nn.BCEWithLogitsLoss(
                                pos_weight=pos_weight, reduction="none"
                            )(cell, lbl)
                ...
                if not tune:
                    tq.set_postfix(
                        loss=np.array(losses).mean(), lr=optimizer.param_groups[0]["lr"]
                    )
            # Validate
            if epoch % cfg.train.validate_every == 0:
                validate_loss, accuracy, auc = basic_validate(
                    model, valid_dl, loss_func, cfg, epoch, tune
                )
                # print(f'type: {type(auc)}, auc:{auc}') #list

                print(
                    (
                        "[ √ ] epochs: {}, train loss: {:.4f}, valid img loss: {:.4f}, "
                        + "valid cell loss: {:.4f}, accuracy: {:.4f}, auc: {:.4f}"
                    ).format(
                        epoch,
                ...
                writer.add_scalar(
                    "train_f{}/loss".format(cfg.experiment.run_fold),
                    np.mean(losses),
                    epoch,
                )
                writer.add_scalar(
                    "train_f{}/lr".format(cfg.experiment.run_fold),
                    optimizer.param_groups[0]["lr"],
                    epoch,
                )
                writer.add_scalar(
                    "valid_f{}/loss_img".format(cfg.experiment.run_fold),
                    validate_loss[0],
                    epoch,
                )
                writer.add_scalar(
                    "valid_f{}/loss_cell".format(cfg.experiment.run_fold),
                    validate_loss[1],
                    epoch,
                )
                writer.add_scalar(
                    "valid_f{}/accuracy".format(cfg.experiment.run_fold),
                    accuracy,
                    epoch,
                )
                writer.add_scalar(
                    "valid_f{}/auc".format(cfg.experiment.run_fold), auc, epoch
                )
            ...
            torch.save(
                model.state_dict(),
                save_path
                / "checkpoints/f{}_epoch-{}.pth".format(cfg.experiment.run_fold, epoch),
            )
        ...
        torch.save(
            model.state_dict(),
            save_path / "checkpoints/quit_f{}.pth".format(cfg.experiment.run_fold),
        )
    ...
    with torch.no_grad():
        ...
        for i, (ipt, mask, lbl, cnt, n_cell) in enumerate(dl):
            ...
            if cfg.basic.amp == "Native":
                with torch.cuda.amp.autocast():
                    ...
                    loss_img_bce = torch.nn.BCEWithLogitsLoss(
                        pos_weight=pos_weight, reduction="none"
                    )(loss_img, lbl)
            ...
            for cell_idx in range(cfg.experiment.num_cells):
                if cfg.basic.amp == "Native":
                    with torch.cuda.amp.autocast():
                        ...
                        loss_cell_bce = torch.nn.BCEWithLogitsLoss(
                            pos_weight=pos_weight, reduction="none"
                        )(loss_cell.squeeze(0), lbl[0])
        ...
        for j in range(len(dl)):
            for k in range(cfg.experiment.num_cells):
                ...
                predicted[j][k] = (
                    cfg.experiment.img_weight * np.array(predicted_img[j])
                    + (1 - cfg.experiment.img_weight) * predicted_cell[cell_ind]
                )
            ...
            accuracy += (
                ((predicted[j] > 0.5) == truth_acc).sum().astype(np.float64)
                / truth_acc.shape[0]
                / truth_acc.shape[1]
            )
        ...
        for img in range(len(dl)):
            ...
            predicted_with_ids.append(
                np.column_stack(
                    (
                        np.broadcast_to(np.array(row_ids[img]), (10, 1)).tolist(),
                        predicted[start:end, :],
                    )
                )
            )
            truth_with_ids.append(
                np.column_stack((row_ids[img], np.expand_dims(truth[img], 0)))
            )

        predicted_with_ids = np.array(predicted_with_ids).reshape(
            (len(dl) * n_cell, 20)
        )  # 20, because 19 labels + ID
        truth_with_ids = np.array(truth_with_ids).reshape(
            (len(dl), 20)
        )  # 20, because 19 labels + ID
        ...
        base_path = Path(cfg.train.dir).joinpath(cfg.basic.id)
        pred_path = base_path.joinpath("pred.csv")
        truth_path = base_path.joinpath("truth.csv")
...
def tta_validate(mdl, dl, loss_func, tta):
    ...
    with torch.no_grad():
        ...
        print("Validation: loss: {:.4f}, kappa: {:.4f}".format(val_loss, val_kappa))
