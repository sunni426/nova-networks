...
def get_best_model():
    ...
    if True:
        ...
        df = pd.read_csv("results/" + cfg.basic.id + "/train.log", sep="\t")
        if args.epoch > 0:
        ...
        test_dl = get_dataloader(cfg)(cfg).get_dataloader(
            test_only=True, tta=args.tta, tta_tfms=None
        )
        _, valid_dl, _ = get_dataloader(cfg)(cfg).get_dataloader(
            tta=args.tta, tta_tfms=None
        )
        # loading model
        model = get_model(cfg)
        model.load_state_dict(
            torch.load(
                ...
                / cfg.basic.id
                / "checkpoints"
                / "f0_epoch{}.pth".format(cfg.experiment.run_fold, best_epoch),
                map_location={
                    "cuda:0": "cpu",
                    "cuda:1": "cpu",
                    "cuda:2": "cpu",
    ...
    with torch.no_grad():
        ...
        preprocess = Compose(
            [
                ...
                ToTensor(),
                Normalize(
                    mean=[0.0979, 0.06449, 0.062307, 0.098419],
                    std=[0.14823, 0.0993746, 0.161757, 0.144149],
                    inplace=True,
                ),
            ]
        )

        images_dir = "/dlab/ldrive/CBT/USLJ-DSDE_DATA-I10008/shihch3/projects/HPA_single_data/code_test/cell"
        file_names = os.listdir(images_dir)
        examples = [
            [os.path.join(images_dir, file_name), 0.4]
            for file_name in np.random.choice(file_names, size=10, replace=False)
        ]
        print(examples)
        ...
        iface = gr.Interface(
            ...
            inputs=[
                ...
                gr.Slider(
                    ...
                    value=0.5,
                    label="Threshold",
                    info="Select the cut-off threshold for a node to be considered as a valid output.",
                ),
            ],
            outputs=[
