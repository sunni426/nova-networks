...
class novaGradCAM:
    ...
    def load(self):
        ...
        image_prep = preprocess_image(
            img,
            mean=[0.08323, 0.05766, 0.0554, 0.08365],
            std=[0.13553, 0.09504, 0.14648, 0.1332],
        )  # .requires_grad_(True)
        ...
        return (
            img,
            image_prep,
        )  # img: (4, 256, 256), image_prep: torch.Size([1, 4, 256, 256])
    ...
    def make_gradcam(self, image, image_prep, folder_path, vit=False):
        ...
        with torch.enable_grad():
            ...
            for i in range(19):
                ...
                grayscale_cam = cam(
                    input_tensor=image_prep.to(self.device), targets=targets
                )
                ...
                if heatmap.shape[-1] < img.shape[-1]:
                    ...
                    heatmap = np.pad(
                        heatmap, ((0, 0), (0, 0), (0, pad_channels)), mode="constant"
                    )
