...
def find_valid_crops(image, masks, save_encode_mask, min_size=(100, 100), max_save=10):
    ...
    for mask_id in unique_masks:
        ...
        for c in range(num_channels):  # Copy color channels
            cropped_cell[..., c] = (
...
def crop_img(args):
    ...
    success_ids = []  # List to store IDs of successful crops
    ...
    if n_CPU != 1:
        with mp.Pool(n_CPU) as pool:
            results = pool.map(_crop_img, id_list)
        for id, success in results:
            if success:
                success_ids.append(id)
    else:
        for id in tqdm(id_list):
            success = _crop_img(id)
            if success:
                success_ids.append(id)
    
    # Filter the original DataFrame to only include successful IDs
    filtered_df = id_df[id_df['ID'].isin(success_ids)]
